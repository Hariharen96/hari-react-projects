...
const initialState = {
    ...
    cart: [], // id,title,price,image,description, qty
    currentitem: null
}


export const shoppingreducers = (state = initialState, action) => {
    switch (action.type) {
        ...
        case shoppingtypes.DELETE_FROM_CART:
            return {
                ...state,
                cart: state.cart.filter(item => item.id !== action.payload.id)
            }
        case shoppingtypes.ADJUST_QUANTITY:
            return {
                ...state,
                cart: state.cart.map(item => item.id === action.payload.id ? {...item, qty: +action.payload.qty } : item)
            }
        case shoppingtypes.LOAD_CURRENT_QUANTITY:
            return {
                ...state,
